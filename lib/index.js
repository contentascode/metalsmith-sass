  ...
  var sass = require('node-sass'),
    ...
    compile = function(config, basePath, files, filename, done) {
      var file = files[filename],
        filePath = path.isAbsolute(filename) ? filename : path.join(basePath, filename),
        opts = extend(
          {
            includePaths: [],
        ...
        computedOutputDir =
          typeof opts.outputDir === 'function' ? opts.outputDir(path.dirname(filename)) : opts.outputDir,
      ...
      if (isAnySassFile(filename) === true) {
        ...
        // Also add _sass folder to includePaths
        opts.includePaths.push(path.join(fileDir, '_sass'));
        ...
        sass.render(opts, function(err, result) {
          ...
          if (err && err instanceof Error) {
            error = new Error(
              ['[metalsmith-sass] Error: ', err.message, ' -> ', err.file, ':', err.line, ':', err.column].join('')
            );
          } else if (err) {
            error = new Error(err);
          }
    ...
    compileSass = function compileSass(config, files, metalsmith, done) {
      ...
      var directory = metalsmith.dir || metalsmith._directory,
        ...
        basePath = config.sources
          ? path.join(directory, config.sources)
      ...
      if (config.sources) {
        var dirFiles = walkSync(path.join(directory, config.sources));
        sourceFiles = dirFiles.reduce(
          (acc, pth) => (
            (acc[pth] = {
